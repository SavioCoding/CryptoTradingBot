...
'''
def get_historical_data (start_time, end_time, symbol="BTCUSDT", interval="1d"):
    bars = client.get_historical_klines(symbol=symbol, interval=interval, start_str=str(start_time), end_str=str(end_time))
...
if __name__ == '__main__':
    ...
    symbols = ["LTCUSDT", "TRXUSDT", "XRPUSDT", "BNBUSDT"]
    ...
    for i in range(len(symbols)):
        for j in range(len(intervals)):
            ...
            start = pd.to_datetime(client._get_earliest_valid_timestamp(symbol=symbol, interval=interval), unit="ms")
            end = datetime.datetime.utcnow()
            data = get_historical_data(start, end, symbol, interval)
